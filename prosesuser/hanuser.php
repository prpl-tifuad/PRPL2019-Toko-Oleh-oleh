z<?php date_default_timezone_set('Asia/Jakarta');
if(empty($_GET['thisposition'])){
	...
	if($to=='registeruser'){
		...
		if($cek>0){?>
		<script>alert("Maaf Username atau Email sudah terdaftar");window.location="../logreg";</script>
		<?php }
		else{
		...
		//
		$ru->insertcheckout($con,$_POST['id_checkout'],$_POST['id_user'],$_POST['an'],$_POST['alamat']/*,$prov['nama'],$kab['nama'],$kec['nama'],$kel['nama'],$_POST['kodepos']*/,$_POST['phone'],$ongcart['total']/*,$prov['ongkir']*/,$ongberat['ongberat'],date('Y-m-d'));
	}
		...
		$ongberat=mysqli_fetch_array(mysqli_query($con,"SELECT sum(ongberat) as ongberat from cart where id_user='$_POST[id_user]' "));
		//
		$ru->updatecheckout($con,$_POST['id_checkout'],$_POST['id_user'],$_POST['an'],$_POST['alamat'],$_POST['phone'],$ongcart['total'],$ongberat['ongberat'],date('Y-m-d'));
	}
